    ...
    <section v-else-if="viewMode==='grid'" class="grid">
      <article v-for="c in filtered" :key="c.idConferencia" class="card" :data-status="store.statusOf(c)">
        ...
        <div class="actions">
          ...
          <a v-else-if="canEvaluate(c)" class="btn secondary" :href="c.evaluacion" target="_blank" rel="noopener">
          ...
          <li v-for="c in group.items" :key="c.idConferencia" class="slot" :data-status="store.statusOf(c)">
            ...
            <div class="slot-content">
              <div class="slot-head">
                ...
                <div class="right">
                  <button class="btn sm secondary" @click="show(c)"><i class="bi bi-eye"></i> Detalles</button>
                  <a v-if="canJoinZoom(c)" class="btn sm primary" :href="c.zoomUrl" target="_blank" rel="noopener">
                    <i class="bi bi-camera-video"></i> Zoom
                  </a>
                  <a v-else-if="canEvaluate(c)" class="btn sm secondary" :href="c.evaluacion" target="_blank" rel="noopener">
                    <i class="bi bi-ui-checks-grid"></i> Evaluación
                  </a>
                </div>
              </div>
...
.search{ display:flex; align-items:center; gap:8px; background:#fff; border:1px solid #e5e7eb; border-radius:12px; padding:8px 10px; width:min(400px,100%); }
.search input{ border:none; outline:none; background:transparent; width:100%; }

.filters{ display:flex; gap:8px; }
.filters select{ border:1px solid #d1d5db; border-radius:8px; padding:6px 8px; font-size:14px; background:#fff; }

.actions{ display:flex; gap:8px; align-items:center; }

.chips{ display:flex; gap:8px; flex-wrap:wrap; margin-top:8px; }
.chip{ background:#ede9fe; color:#4c1d95; padding:4px 10px; border-radius:999px; font-size:13px; display:inline-flex; align-items:center; gap:6px; cursor:default; }
.chip i{ cursor:pointer; }

.btn{ border:1px solid #d1d5db; padding:8px 12px; border-radius:8px; cursor:pointer; font-weight:600; display:inline-flex; align-items:center; gap:6px; }
.btn.primary{ background:linear-gradient(135deg,#7c3aed,#6d28d9); color:#fff; border:none; }
.btn.secondary{ background:#f3f4f6; color:#374151; }
.btn.sm{ padding:6px 10px; font-size:13px; }

.view-toggle{ display:flex; gap:6px; }
.toggle-btn{ border:1px solid #d1d5db; background:#fff; width:40px; height:40px; border-radius:8px; cursor:pointer; display:grid; place-items:center; }
.toggle-btn.active{ background:linear-gradient(135deg,#7c3aed,#6d28d9); color:#fff; border:none; }
...
.card{ background:#fff; border:1px solid #e5e7eb; border-radius:12px; box-shadow:0 6px 20px rgba(0,0,0,.06); padding:16px; display:grid; gap:10px; }
.card-head{ display:grid; gap:4px; }
.status{ font-size:12px; font-weight:600; padding:3px 8px; border-radius:999px; background:#f3f4f6; color:#374151; }
...
.meta{ list-style:none; padding:0; margin:0; display:grid; gap:4px; font-size:13px; color:#374151; }

.timeline{ display:grid; gap:16px; }
.day-group{ background:#fff; border:1px solid #e5e7eb; border-radius:12px; padding:14px; }
.day-title{ margin:0 0 10px; font-size:15px; font-weight:700; color:#4c1d95; }
.line{ list-style:none; padding:0; margin:0; }
.slot{ display:flex; gap:12px; padding:10px 0; border-left:3px solid #e2e8f0; margin-left:12px; position:relative; }
.slot + .slot{ border-top:1px dashed #f1f1f1; }
.dot{ width:12px; height:12px; border-radius:50%; background:#5b6bff; position:absolute; left:-7px; top:16px; }
.slot[data-status="live"] .dot{ background:#0e7490; }
.slot[data-status="finished"] .dot{ background:#64748b; }
.slot-content{ flex:1; display:grid; gap:4px; }
</style>
