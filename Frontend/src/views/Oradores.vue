...
const oradores = ref([
  {
    id: 1,
    nombre: 'Fabrisio Condarco',
    titulo: 'Sr. Ingeniero de Software',
    bio: 'Apasionado por IA y arquitectura.',
    charlas: 3,
    score: 4.8,
    tags: 'IA,Cloud',
  },
  {
    id: 2,
    nombre: 'Jose Manzaneda',
    titulo: 'Security Lead',
    bio: 'Ciberseguridad ofensiva/defensiva.',
    charlas: 2,
    score: 4.7,
    tags: 'SecOps',
  },
  {
    id: 3,
    nombre: 'Miguel Quisbert',
    titulo: 'DevRel',
    bio: 'Comunidad, DX y demos.',
    charlas: 4,
    score: 4.6,
    tags: 'DX,DevRel',
  },
...
function validate() {
  ...
  if (
    String(form.charlas).trim() === '' ||
    Number.isNaN(Number(form.charlas)) ||
    Number(form.charlas) < 0
  )
    return 'Charlas debe ser número ≥ 0'
  if (String(form.score).trim() === '' || Number(form.score) < 0 || Number(form.score) > 5)
    return 'Score debe estar entre 0 y 5'
  return null
}
function save() {
  const err = validate()
      ...
      <div class="row g-2">
        <div class="col-md-6">
          ...
          <input
            ...
            type="search"
            class="form-control"
            placeholder="Nombre, título, tags…"
          />
        </div>
        <div class="col-md-3">
        ...
        <table class="table align-middle mb-0">
          ...
          <tbody>
            <tr v-for="o in paged" :key="o.id">
              <td>
                <div
                  class="rounded-circle border d-inline-grid place-items-center"
                  style="width: 40px; height: 40px; background: #ede9fe; border-color: #e9d5ff"
                >
              ...
              <td class="d-none d-lg-table-cell">
                <span
                  v-for="t in (o.tags || '').split(',').filter(Boolean)"
                  :key="t"
        ...
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title text-danger">
              <i class="bi bi-exclamation-triangle"></i> Eliminar orador
            </h5>
            <button
              type="button"
              class="btn-close"
