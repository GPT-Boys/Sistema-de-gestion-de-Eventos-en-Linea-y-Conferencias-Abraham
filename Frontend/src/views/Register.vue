<template>
  <div class="register-page">
    <!-- Navbar mini con logo que vuelve al Home -->
    <HeaderAuth />
        ...
        <form @submit.prevent="onSubmitRegister" novalidate>
          ...
          <div class="grid-2">
            ...
            <section>
              ...
              <label class="label" for="usuario">Nombre de usuario <span class="req">*</span></label>
              <div class="control">
                <span class="icon left" aria-hidden="true">
                  <svg viewBox="0 0 24 24"><path d="M12 12a5 5 0 1 0-5-5 5 5 0 0 0 5 5Zm0 2c-4.42 0-8 2.24-8 5v2h16v-2c0-2.76-3.58-5-8-5Z"/></svg>
                </span>
                <input id="usuario" v-model.trim="usuario" type="text" placeholder="Tu usuario" required />
              </div>
              ...
              <div class="control">
                ...
                <input
                  ...
                  required
                />
                <button type="button" class="icon right btn-icon"
              ...
              <label class="label" for="confirmPassword">Confirmar contraseña <span class="req">*</span></label>
              <div class="control">
                <span class="icon left" aria-hidden="true">
                  <svg viewBox="0 0 24 24"><path d="M12 1a5 5 0 0 0-5 5v3H5a2 2 0 0 0-2 2v9a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-9a2 2 0 0 0-2-2h-2V6a5 5 0 0 0-5-5Z"/></svg>
                </span>
                <input
                  id="confirmPassword"
                ...
                <button type="button" class="icon right btn-icon"
                  ...
                  <svg v-if="!showConfirmPassword" viewBox="0 0 24 24"><path d="M12 5C7 5 2.73 8.11 1 12c1.73 3.89 6 7 11 7s9.27-3.11 11-7c-1.73-3.89-6-7-11-7Zm0 12a5 5 0 1 1 0-10 5 5 0 0 1 0 10Z"/></svg>
                  <svg v-else viewBox="0 0 24 24"><path d="M2 5.27 3.28 4 20 20.72 18.73 22l-2.27-2.27A11.9 11.9 0 0 1 12 19C7 19 2.73 15.89 1 12a13.1 13.1 0 0 1 5.11-5.92L2 5.27Z"/></svg>
                </button>
              </div>

              <label class="label" for="tipoUsuario">Tipo de usuario <span class="req">*</span></label>
              <div class="control">
                <span class="icon left" aria-hidden="true">
                  <svg viewBox="0 0 24 24"><path d="M16 11a4 4 0 1 0-4-4 4 4 0 0 0 4 4Zm-8 0a4 4 0 1 0-4-4 4 4 0 0 0 4 4Zm0 2c-3.33 0-10 1.67-10 5v3h8v-3c0-1.33.53-2.55 1.39-3.54A9.67 9.67 0 0 0 8 13Zm8 0c-1.61 0-4.67.8-6.14 2.46A5.06 5.06 0 0 0 9 18v3h14v-3c0-3.33-6.67-5-9-5Z"/></svg>
                </span>
                <select id="tipoUsuario" v-model="tipoUsuario" required>
                  <option v-for="o in tiposUsuario" :key="o.value" :value="o.value" :disabled="o.disabled">{{ o.text }}</option>
                </select>
              </div>
            </section>
            ...
            <section>
              ...
              <div class="control">
                <span class="icon left" aria-hidden="true">
                  <svg viewBox="0 0 24 24"><path d="M12 2C8.14 2 5 5.14 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.86-3.14-7-7-7Zm0 9.5a2.5 2.5 0 1 1 2.5-2.5A2.5 2.5 0 0 1 12 11.5Z"/></svg>
                </span>
                <select id="ciudad" v-model="ciudad" required>
                  <option v-for="d in departamentosBolivia" :key="d.value" :value="d.value" :disabled="d.disabled">{{ d.text }}</option>
                </select>
              </div>
              ...
              <div class="control">
                <span class="icon left" aria-hidden="true">
                  <svg viewBox="0 0 24 24"><path d="M6.62 10.79a15 15 0 0 0 6.59 6.59l2.2-2.2a1 1 0 0 1 1-.24 11.36 11.36 0 0 0 3.56.57 1 1 0 0 1 1 1V20a1 1 0 0 1-1 1A16 16 0 0 1 3 5a1 1 0 0 1 1-1h3.5a1 1 0 0 1 1 1 11.36 11.36 0 0 0 .57 3.56 1 1 0 0 1-.26 1Z"/></svg>
                </span>
          ...
          <label class="check">
            ...
            <span>
              Acepto los
              <button type="button" class="link-inline" @click="openModal('terms')">términos y condiciones</button>
              y la
              <button type="button" class="link-inline" @click="openModal('privacy')">política de privacidad</button>
              <span class="req">*</span>
            </span>
          </label>
    ...
    <!-- Modal (teleport a body para evitar conflictos con Bootstrap/z-index) -->
    <teleport to="body">
      <div v-if="showModal" class="aes-backdrop" @click.self="closeModal">
        <div class="aes-modal" role="dialog" aria-modal="true" :aria-label="modalTitle">
          <header class="modal-head">
            <h3>{{ modalTitle }}</h3>
            <button type="button" class="btn-icon" @click="closeModal" aria-label="Cerrar">×</button>
          </header>
          <div class="modal-body">
            <p>{{ modalContent }}</p>
            <div class="placeholder-pdf">
              <svg viewBox="0 0 24 24"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8Zm0 2.5L18.5 9H14Z"/></svg>
              <p>Aquí se mostrará el PDF cuando esté disponible.</p>
            </div>
          </div>
          <footer class="modal-foot">
            <button type="button" class="aes-btn aes-btn--secondary" @click="closeModal">Cerrar</button>
            <button type="button" class="aes-btn aes-btn--primary" @click="closeModal">Aceptar</button>
          </footer>
        </div>
      </div>
    </teleport>
...
const showNotification = (message, type) => {
  ...
  setTimeout(() => { if (notification.value.message === message) notification.value.message = '' }, 5000)
}
...
const onSubmitRegister = async () => {
  ...
  if (password.value !== confirmPassword.value) return showNotification('Las contraseñas no coinciden.', 'error')
  if (!acceptTerms.value) return showNotification('Debes aceptar los términos y condiciones.', 'error')
...
/* Página con gradiente tipo login + espacio para header fijo */
.register-page{
  min-height:100svh;
  background:
  ...
  position:relative; padding-top: var(--header-h, 80px);
}
...
/* Card grande con scroll interno */
.register-card{
  width:min(960px, 94vw);
  max-height: calc(94vh - var(--header-h, 80px));
...
.form{ flex:1; overflow:auto; padding:18px 22px 12px; }
...
/* Grid */
.grid-2{ display:grid; grid-template-columns:1fr 1fr; gap:16px 20px; margin-bottom:10px; }
.grid-2 section{ min-width:0; }
.section-title{ margin:0 0 8px; font-size:15px; font-weight:700; color:#374151; }

.label{ display:block; font-size:13px; color:#374151; margin:8px 0 6px; }
.req{ color:#ef4444; }
.hint{ margin:4px 0 10px; font-size:12px; color:#6b7280; }
...
.control input:focus, .control select:focus{ border-color: var(--purple-600, var(--purple-700)); box-shadow: var(--ring); }
...
/* Checkbox/links */
.check{ display:flex; align-items:flex-start; gap:10px; font-size:14px; color:#374151; margin:8px 0 12px; }
.check input{ width:16px; height:16px; margin-top:2px; }
.link-inline{ background:transparent; border:none; color:var(--purple-600, var(--purple-700)); font-weight:600; cursor:pointer; padding:0; }
...
.aes-btn{
  ...
  text-decoration:none; display:inline-block; text-align:center; margin-top:6px;
}
...
.aes-btn--secondary{ background:#e5e7eb; color:#111827; }
...
/* Modal (sin choque con Bootstrap) */
.aes-backdrop{
  position:fixed; inset:0; background:rgba(0,0,0,.45);
  display:grid; place-items:center; padding:16px; z-index:1100;
}
.aes-modal{
  width:min(640px, 96vw); background:#fff; border-radius:16px; overflow:hidden;
  box-shadow:0 30px 80px rgba(0,0,0,.45);
}
.modal-head{ display:flex; align-items:center; justify-content:space-between; padding:14px 16px; border-bottom:1px solid #f3f4f6; }
.modal-head h3{ margin:0; font-size:18px; }
.modal-body{ padding:16px; color:#374151; }
.modal-foot{ display:flex; gap:10px; justify-content:flex-end; padding:12px 16px; border-top:1px solid #f3f4f6; }
.placeholder-pdf{ display:grid; place-items:center; gap:6px; padding:16px; border:2px dashed #e5e7eb; border-radius:12px; margin-top:10px; }
