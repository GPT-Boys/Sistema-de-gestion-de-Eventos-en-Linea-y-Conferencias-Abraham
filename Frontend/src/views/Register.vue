              ...
              <section>
                ...
                <div class="control">
                  ...
                  <select id="tipoUsuario" v-model="tipoUsuario" required>
                    <option v-for="o in tiposUsuario" :key="o.value" :value="o.value" :disabled="o.disabled">
                      {{ o.text }}
                    </option>
                  </select>
                </div>
...
.form{ flex:1; padding:18px 22px 12px; }
...
.control input:focus, .control select:focus{ border-color: var(--purple-600); box-shadow: var(--ring); }
...
.link-inline{ background:transparent; border:none; color:var(--purple-600); font-weight:600; cursor:pointer; padding:0; }
...
/* Modal */
.aes-backdrop{ position:fixed; inset:0; background:rgba(0,0,0,.45); display:grid; place-items:center; padding:16px; z-index:1100; }
.aes-modal{ width:min(640px, 96vw); background:#fff; border-radius:16px; overflow:hidden; box-shadow:0 30px 80px rgba(0,0,0,.45); }
