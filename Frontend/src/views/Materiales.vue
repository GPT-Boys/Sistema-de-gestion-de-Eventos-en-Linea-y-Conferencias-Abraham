...
let nextId = 4
const materials = ref([
  { id:1, nombre:'Agenda general.pdf',    tipo:'pdf',   size: 245_000, relacionado:'Evento',    visible:true,  url:null, uploadedAt: new Date() },
  { id:2, nombre:'Keynote-Cloud.pptx',    tipo:'ppt',   size: 3_240_000, relacionado:'Keynote', visible:true,  url:null, uploadedAt: new Date() },
  { id:3, nombre:'logo.png',              tipo:'image', size: 56_000, relacionado:'Branding',   visible:false, url:null, uploadedAt: new Date() },
])
...
function detectType(file) {
  ...
  if (t.includes('presentation') || file.name.toLowerCase().endsWith('.ppt') || file.name.toLowerCase().endsWith('.pptx')) return 'ppt'
...
function iconFor(t) {
  switch(t){
    ...
    case 'ppt': return 'bi-file-earmark-slides-fill text-warning'
    case 'doc': return 'bi-file-earmark-text-fill text-primary'
    case 'image': return 'bi-file-earmark-image-fill text-success'
...
function openEdit(m){
  Object.assign(current, { id:m.id, nombre:m.nombre, relacionado:m.relacionado, visible:m.visible })
  showEdit.value = true
}
function saveEdit(){
  const idx = materials.value.findIndex(m => m.id === current.id)
  if (idx !== -1){
    materials.value[idx] = { ...materials.value[idx], nombre: current.nombre.trim() || materials.value[idx].nombre, relacionado: current.relacionado, visible: current.visible }
  }
...
onUnmounted(() => {
  materials.value.forEach(m => { if (m.url) URL.revokeObjectURL(m.url) })
})
...
const filtered = computed(() => {
  ...
  return materials.value.filter(m => {
    ...
    const okText = !term || m.nombre.toLowerCase().includes(term) || (m.relacionado || '').toLowerCase().includes(term)
  ...
  <main id="main" tabindex="-1" class="page d-grid gap-3">
    <header class="d-flex align-items-center justify-content-between gap-2">
      <div>
        ...
        <p class="text-muted m-0">Biblioteca de archivos (solo frontend). Los subidos aquí no persisten tras refrescar.</p>
      </div>
    ...
    <div class="card p-3">
      <div class="row g-2">
        <div class="col-md-6">
          ...
          <input v-model="q" type="search" class="form-control" placeholder="Nombre o relacionado…" @input="page=1">
        </div>
        <div class="col-md-3">
    ...
    <div class="card">
          ...
          <tbody>
            <tr v-for="m in paged" :key="m.id">
              ...
              <td>
                ...
                <div class="small text-muted d-md-none">{{ m.relacionado || '—' }} · {{ fmtSize(m.size) }}</div>
              </td>
              <td class="d-none d-md-table-cell">{{ m.relacionado || '—' }}</td>
              <td class="text-end d-none d-md-table-cell">{{ fmtSize(m.size) }}</td>
              <td class="text-end">
                <div class="btn-group">
                  <a v-if="m.url" class="btn btn-sm btn-outline-secondary" :href="m.url" :download="m.nombre" title="Descargar">
                    <i class="bi bi-download"></i>
                  </a>
                  <button class="btn btn-sm btn-outline-secondary" @click="openEdit(m)" title="Editar">
      ...
      <div class="d-flex align-items-center justify-content-between p-3 border-top">
        <div class="text-muted small">Mostrando {{ paged.length }} de {{ filtered.length }} archivos</div>
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            ...
            <button type="button" class="btn-close" @click="showEdit=false" aria-label="Cerrar"></button>
          </div>
          <div class="modal-body">
            ...
            <div class="mb-3">
              ...
              <input v-model="current.relacionado" type="text" class="form-control" placeholder="Ej: Keynote, Taller A, etc.">
            </div>
            <div class="form-check">
              <input v-model="current.visible" id="visible" class="form-check-input" type="checkbox">
          ...
          <div class="modal-footer">
            ...
            <button class="btn btn-primary" @click="saveEdit"><i class="bi bi-check-lg"></i> Guardar</button>
          </div>
        </div>
      </div>
    </div>
