...
const tab = ref(localStorage.getItem('exp-tab') || 'activas') // 'activas' | 'finalizadas'
...
// 👇 aquí usamos el getter activeList
const baseList = computed(() => {
  return tab.value === 'finalizadas'
    ? store.finishedList
    : store.activeList
})
  ...
  <div class="page">
    <header class="head">
      <div>
        ...
        <p class="muted">Busca e inscríbete en charlas activas. Puedes ver finalizadas en la pestaña aparte.</p>
      </div>

      <div class="toolbar">
        <!-- Tabs -->
        <div class="tabs">
          <button :class="['tab',{active:tab==='activas'}]" @click="setTab('activas')">
            Activas (Próximas / En curso)
          </button>
          <button :class="['tab',{active:tab==='finalizadas'}]" @click="setTab('finalizadas')">
            Finalizadas
          </button>
        </div>
...
<style scoped>
/* Usa los mismos estilos que ya tenías, solo se añadieron tabs */
</style>
...
.tab{ border:1px solid var(--b); background:#6b7280; padding:10px 16px; border-radius:12px; cursor:pointer; font-weight:700; }
.tab.active{ background: #6d28d9; color:#000000; border:none; }
