    ...
    <section v-else-if="viewMode==='grid'" class="grid">
      <article v-for="c in filtered" :key="c.idConferencia" class="card" :data-status="store.statusOf(c)">
        ...
        <div class="actions">
          ...
          <!-- Solo permitir cancelar inscripción si aún es PRÓXIMA -->
          <button
        ...
        <!-- VoteWidget: solo si está inscrito, live/finished y NO votó -->
        <VoteWidget
            ...
            <div class="slot-content">
              <div class="slot-head">
                ...
                <div class="right">
                  ...
                  <!-- Solo PRÓXIMAS permiten cancelar -->
                  <button
                    v-if="store.statusOf(c)==='upcoming'"
                    class="btn sm enroll"
                    :class="{ on:isEnrolled(c) }"
...
.btn.primary{ background:var(--grad); color:#6d28d9; border:none; }
.btn.enroll.on{ background:#faf5ff; color:#6d28d9; border:1px solid #e9d5ff; }
