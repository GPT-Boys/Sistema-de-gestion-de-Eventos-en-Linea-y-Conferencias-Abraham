...
const kpis = [
  { id: 'eventos',     label: 'Eventos activos',      value: 12,   delta: '+8%',   series: [8,9,10,9,11,12] },
  { id: 'charlas',     label: 'Charlas hoy',         value: 24,   delta: '+2',    series: [20,18,19,22,21,24] },
  { id: 'asistentes',  label: 'Asistentes registrados', value: 438, delta: '+32',  series: [300,320,360,390,410,438] },
  { id: 'satisfaccion',label: 'Satisfacción (NPS)',  value: 72,   delta: '+4',    series: [66,68,67,69,70,72] },
]

const proximas = [
  { id: 1, titulo: 'Apertura: Tendencias IA 2025', sala: 'Auditorio A', fecha: 'Hoy 10:30', orador: 'Dra. Valdez' },
  { id: 2, titulo: 'Panel: Ciberseguridad',        sala: 'Sala 2',      fecha: 'Hoy 12:00', orador: 'Equipo SecOps' },
  { id: 3, titulo: 'Taller: Prompt Engineering',   sala: 'Lab 1',       fecha: 'Hoy 15:00', orador: 'Ing. R. Rivera' },
  { id: 4, titulo: 'Keynote: Cloud Native',        sala: 'Auditorio B', fecha: 'Mañana 09:00', orador: 'Invitado Intel' },
]
...
const donutStyle = computed(() => {
  ...
  const segments = participacion.value.map((s, idx) => {
  ...
  <div class="dash">
    ...
    <header class="dash-head">
      ...
      <div class="actions">
        ...
        <button class="btn btn-primary">
          <i class="bi bi-plus-lg"></i> Nueva conferencia
        </button>
      </div>
    </header>
    ...
    <section class="kpi-grid" aria-label="Indicadores clave">
      <article v-for="k in kpis" :key="k.id" class="kpi">
        <header>
          <h3>{{ k.label }}</h3>
          <span class="delta" :class="{ up: String(k.delta).includes('+'), down: String(k.delta).includes('-') }">
            {{ k.delta }}
          </span>
        </header>
        <div class="kpi-main">
          ...
          <svg class="spark" viewBox="0 0 100 30" preserveAspectRatio="none" aria-hidden="true">
            <!-- línea -->
            <polyline
              :points="k.series.map((v, i) => `${(i/(k.series.length-1))*100},${30 - (v/Math.max(...k.series))*26 - 2}`).join(' ')"
              fill="none" stroke="currentColor" stroke-width="2" />
            <!-- relleno suave -->
            <polygon
              :points="(() => {
                const pts = k.series.map((v, i) => `${(i/(k.series.length-1))*100},${30 - (v/Math.max(...k.series))*26 - 2}`)
          ...
          <div class="donut-wrap">
            ...
            <ul class="legend">
              <li v-for="(s, idx) in participacion" :key="s.label">
                <span class="swatch" :style="{ backgroundColor: ['#6d28d9','#8b5cf6','#a78bfa','#c4b5fd'][idx%4] }"></span>
...
/* ===== Tokens ===== */
:root{
  --paper:#fff; --ink:#0b0b0d; --muted:#6b7280;
  --purple-900:#4c1d95; --purple-700:#6d28d9; --purple-500:#8b5cf6;
  --ring: 0 0 0 3px rgba(109,40,217,.22);
}
...
.filter select{ border:none; background:transparent; outline:none; font-weight:600; }
.btn{
...
.kpi{
  background:#ffffff; border:1px solid #eef0f4; border-radius:16px; padding:12px;
  box-shadow: 0 10px 30px rgba(17,24,39,.06);
}
.kpi header{ display:flex; align-items:center; justify-content:space-between; }
.kpi h3{ margin:0; font-size:13px; color:#475569; font-weight:700; }
.delta{ font-size:12px; padding:3px 8px; border-radius:999px; background:#f1f5f9; color:#0f172a; }
.delta.up{ background:#ecfdf5; color:#065f46; }
.delta.down{ background:#fef2f2; color:#7f1d1d; }
.kpi-main{ display:flex; align-items:flex-end; gap:8px; margin-top:8px; }
.kpi-value{ font-size:28px; font-weight:800; line-height:1; }
.spark{ width:120px; height:42px; color: var(--purple-700); }
.spark-fill{ fill: rgba(109,40,217,.12); }

/* ===== Grid 2 cols ===== */
.grid-2{ display:grid; gap:12px; grid-template-columns: 1.25fr 1fr; }
...
.link{ font-size:13px; color: var(--purple-700); text-decoration:none; font-weight:700; }

.list{ list-style:none; padding:0; margin:0; display:grid; gap:8px; }
.row{ display:flex; align-items:center; justify-content:space-between; gap:10px; padding:10px; border-radius:12px; background:#fafbff; border:1px solid #f1f5f9; }
.row:hover{ background:#f7f8fe; }
.meta{ min-width:0; }
.title{ font-weight:700; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
.sub{ font-size:12px; color:#64748b; }
.aux{ display:flex; align-items:center; gap:6px; color:#0f172a; font-weight:700; }

/* Sesiones */
.list-sessions .row .aux i{ color: var(--purple-700); }

/* Timeline */
.timeline{ list-style:none; padding:0; margin:0; display:grid; gap:8px; }
.timeline li{ position:relative; display:flex; gap:10px; }
.timeline .dot{
  width:10px; height:10px; border-radius:50%; background:var(--purple-700); margin-top:8px; flex:0 0 auto;
}
.t-body{ background:#fafbff; border:1px solid #f1f5f9; border-radius:12px; padding:10px; flex:1; }
.t-text{ font-size:14px; }
.t-time{ font-size:12px; color:#64748b; margin-top:4px; }
...
.legend li{ display:grid; grid-template-columns:auto 1fr auto; align-items:center; gap:8px; }
.swatch{ width:10px; height:10px; border-radius:3px; }
.lg-label{ font-size:13px; color:#0f172a; }
.lg-val{ font-weight:800; }
...
.list-speakers .avatar img{ width:100%; height:100%; object-fit:cover; display:block; }
.list-speakers .fallback{ width:100%; height:100%; display:grid; place-items:center; background:#ede9fe; color:#4c1d95; font-weight:800; }
.list-speakers .row{ gap:12px; }

/* Quick actions */
.quick{ display:grid; grid-template-columns: repeat(2, minmax(0,1fr)); gap:10px; }
.qbtn{
  border:1px solid #e5e7eb; background:#fff; border-radius:12px; padding:12px; text-align:left; font-weight:700; cursor:pointer;
