...
const sesiones = ref([])
onMounted(()=>{ try{ const raw=localStorage.getItem(STORAGE_KEY); if(raw) sesiones.value=JSON.parse(raw) }catch{} })
watch(sesiones, v=>{ try{ localStorage.setItem(STORAGE_KEY, JSON.stringify(v)) }catch{} }, {deep:true})
...
const filtered = computed(()=>{
  ...
  return sesiones.value.filter(s=>{
    const okText = !term || [s.titulo,s.orador,s.sala,s.track].some(v=> String(v||'').toLowerCase().includes(term))
...
watch([q,sala,track,estado,()=>rango.desde,()=>rango.hasta,sortKey,sortDir,pageSize], ()=>{ page.value=1 })

/** ===== Crear/editar ===== */
const showModal = ref(false)
const form = reactive({
  id:null, titulo:'', orador:'', sala:'', track:'', descripcion:'',
  fecha:'', inicio:'10:00', fin:'11:00',
  cupo:100, inscritos:0, estado:'Programada', destacado:false
})
const showDelete = ref(false)
const toDelete = ref(null)

function openCreate(){
  Object.assign(form, { id:null, titulo:'', orador:'', sala:'', track:'', descripcion:'',
    fecha: new Date().toISOString().slice(0,10), inicio:'10:00', fin:'11:00', cupo:100, inscritos:0, estado:'Programada', destacado:false })
  showModal.value = true
}
function openEdit(s){ Object.assign(form, s); showModal.value = true }
function validate(){
...
function askDelete(s){ toDelete.value = s; showDelete.value = true }
...
function toggleStar(s){
  ...
  if (i!==-1) sesiones.value[i] = { ...sesiones.value[i], destacado: !sesiones.value[i].destacado }
}
function occupancy(s){
  const c = Number(s.cupo)||0, ins=Number(s.inscritos)||0
  if (c<=0) return '—'
...
<template>
  <main id="main" tabindex="-1" class="d-grid gap-3">
    <header class="d-flex align-items-center justify-content-between gap-2">
      ...
      <button class="btn btn-primary" @click="openCreate"><i class="bi bi-plus-lg"></i> Nueva charla</button>
    </header>
    ...
    <div class="card p-3">
      <div class="row g-2">
        <div class="col-lg-4">
          ...
          <input v-model="q" type="search" class="form-control" placeholder="Título, orador, sala, track…">
        </div>
        ...
        <div class="col-sm-6 col-lg-2">
          ...
          <div class="input-group">
            ...
            <button class="btn btn-outline-secondary" @click="sortDir = sortDir==='asc'?'desc':'asc'">
    ...
    <div class="card">
        ...
        <table class="table align-middle mb-0">
          ...
          <tbody>
            <tr v-for="s in paged" :key="s.id">
              <td>
                <button class="btn btn-sm btn-link p-0" :title="s.destacado?'Quitar destacado':'Marcar destacado'" @click="toggleStar(s)">
              ...
              <td>{{ s.fecha }} <span class="text-muted small">{{ s.inicio }}–{{ s.fin }}</span></td>
              <td class="text-end d-none d-md-table-cell">{{ s.sala }}</td>
              <td class="text-end">{{ occupancy(s) }}</td>
              <td class="text-end">
                <div class="btn-group">
                  <button class="btn btn-sm btn-outline-secondary" @click="openEdit(s)"><i class="bi bi-pencil-square"></i></button>
                  <button class="btn btn-sm btn-outline-danger" @click="askDelete(s)"><i class="bi bi-trash"></i></button>
                </div>
              </td>
            </tr>
      ...
      <div class="d-flex align-items-center justify-content-between p-3 border-top">
        <div class="text-muted small">Mostrando {{ paged.length }} de {{ sorted.length }} charlas</div>
        ...
        <div class="modal-body">
          <div class="row g-3">
            ...
            <div class="col-12 form-check">
              <input id="dest" v-model="form.destacado" class="form-check-input" type="checkbox">
    ...
    <div v-if="showDelete" class="modal d-block" tabindex="-1" aria-modal="true" role="dialog">
      <div class="modal-dialog"><div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title text-danger"><i class="bi bi-exclamation-triangle"></i> Eliminar charla</h5>
