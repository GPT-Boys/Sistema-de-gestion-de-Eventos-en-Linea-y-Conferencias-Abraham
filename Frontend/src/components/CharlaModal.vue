          ...
          <div class="aes-modal__dialog" role="document" ref="dialogRef">
            ...
            <section class="aes-modal__body">
              ...
              <ul class="meta">
                ...
                <li><i class="bi bi-clock"></i>{{ fmtHora(charla.horaEmpieza) }} – {{ fmtHora(charla.horaTermina) }}</li>
                ...
                <li v-if="typeof charla.votosAFavor === 'number'">
                  <i class="bi bi-hand-thumbs-up"></i>{{ charla.votosAFavor }} 👍 · {{ charla.votosEnContra }} 👎
                </li>
              </ul>

              <div class="cta">
                <div class="cta-left">
                  <a v-if="charla.zoomUrl && canJoinZoom" class="btn primary" :href="charla.zoomUrl" target="_blank" rel="noopener">
                    <i class="bi bi-camera-video"></i> Unirse por Zoom
                  </a>
                  <button v-if="charla.zoomUrl && canJoinZoom" class="btn ghost" type="button" @click="copyZoom">
                    <i class="bi bi-clipboard"></i> Copiar enlace
                  </button>
                  <a v-if="canEvaluate" class="btn" :href="charla.evaluacion" target="_blank" rel="noopener">
                    <i class="bi bi-ui-checks-grid"></i> Evaluación
                  </a>
                  <a v-if="hasMaterial" class="btn" :href="charla.materialUrl" target="_blank" rel="noopener">
...
.icon-btn{ border:none; background:transparent; cursor:pointer; width:36px; height:36px;
