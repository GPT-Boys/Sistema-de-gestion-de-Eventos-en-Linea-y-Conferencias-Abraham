...
.nav a, .nav :deep(a){
  font-weight:600; color:#1f2937; position:relative; padding:4px 0;
}
.nav a::after, .nav :deep(a)::after{
  content:''; position:absolute; left:0; bottom:-6px; width:0; height:2px;
  background:var(--purple-700, #6d28d9); transition:width .2s;
}
.nav a:hover::after, .nav :deep(a:hover)::after{ width:100%; }

/* Estado activo de scroll-spy */
.nav a.active{ color:var(--purple-700, #6d28d9); }
...
@media (max-width:900px){
  .nav{ display:none; flex-direction:column; background:#fff; box-shadow:0 10px 20px rgba(0,0,0,.1);
        padding:12px; border-radius:8px; position:absolute; top:var(--header-h, 140px); right:10px; }
